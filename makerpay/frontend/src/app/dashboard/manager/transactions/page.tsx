...
const sCls = (s: string) => s === 'completed' ? 'bg-green-500/10 text-green-400' : s === 'pending' ? 'bg-yellow-500/10 text-yellow-400' : s === 'refunded' ? 'bg-blue-500/10 text-blue-400' : 'bg-red-500/10 text-red-400';

export default function ManagerTransactionsPage() {
  const [status, setStatus] = useState('');
  const [search, setSearch] = useState('');

  const { data: raw } = useQuery({ queryKey: ['manager-payments', status], queryFn: () => adminApi.getPayments({ status: status || undefined, limit: 50 }), retry: false });
  const payments: any[] = (raw as any)?.data || (Array.isArray(raw) ? (raw as any[]) : MOCK);
  const filtered = payments.filter(p => (!status || p.status === status) && (!search || p.externalOrderId?.includes(search) || p.customerName?.toLowerCase().includes(search.toLowerCase())));
      ...
      <div className="bg-[#111] border border-white/10 rounded-2xl overflow-hidden">
        <div className="flex items-center gap-4 px-6 py-4 border-b border-white/10 flex-wrap">
          <div className="flex items-center gap-1 bg-white/5 rounded-xl p-1">
            {[{ label: 'Barchasi', value: '' }, { label: 'Muvaffaqiyatli', value: 'completed' }, { label: 'Kutmoqda', value: 'pending' }, { label: 'Xato', value: 'failed' }].map(o => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/10">{['Buyurtma', 'Merchant', 'Mijoz', 'Provayder', 'Summa', 'Status', 'Sana'].map(h => <th key={h} className="py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wide text-left">{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(p => (
